...
export const ShareMilestoneModal: React.FC<ShareMilestoneModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-md rounded-2xl border border-[var(--border-subtle)] bg-[var(--card-surface)] p-6 shadow-2xl animate-in zoom-in-95 duration-200">
        <button
          onClick={onClose}
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="font-heading font-bold text-base text-[var(--text-primary)]">Share Milestone</h3>
            <p className="text-xs text-[var(--text-secondary)] flex items-center gap-1 mt-0.5">
              <ShieldCheck className="h-3.5 w-3.5 text-emerald-700 dark:text-emerald-400" /> Private titles only (No task details leaked)
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { id: 'ring_closed', label: 'Rings Closed', icon: Trophy, color: 'text-amber-700 dark:text-amber-400 bg-amber-500/10 border-amber-500/30' },
                { id: 'streak_milestone', label: 'Streak Level', icon: Flame, color: 'text-orange-700 dark:text-orange-400 bg-orange-500/10 border-orange-500/30' },
                { id: 'focus_marathon', label: 'Sprint Marathon', icon: Zap, color: 'text-emerald-700 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/30' },
                { id: 'habit_mastered', label: 'Habit Mastery', icon: Target, color: 'text-teal-700 dark:text-teal-400 bg-teal-500/10 border-teal-500/30' },
              ].map((item) => {
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 rounded-xl border p-2.5 text-xs font-medium transition ${
                      isSelected
                        ...
                        : 'border-[var(--border-subtle)] bg-[var(--surface-sunken)] text-[var(--text-secondary)] hover:border-[var(--border-subtle)] hover:text-[var(--text-primary)]'
                    }`}
                  >
          ...
          <div>
            ...
            <input
              ...
              required
              className="w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-sunken)] p-3 text-xs text-[var(--text-primary)] placeholder-[var(--text-muted)] focus:border-[var(--accent-botanical-sage)] focus:outline-none"
            />
          </div>
          ...
          <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3 flex items-center gap-2.5">
            <ShieldCheck className="h-4 w-4 text-emerald-700 dark:text-emerald-400 shrink-0" />
            <p className="text-[11px] text-emerald-800 dark:text-emerald-200 leading-tight">
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl border border-[var(--border-subtle)] bg-[var(--card-hover)] px-4 py-2 text-xs font-semibold text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition"
            >
              Cancel
            </button>
            <button
