    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg rounded-2xl border border-[var(--border-subtle)] bg-[var(--card-surface)] p-6 shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center gap-3 mb-5">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 shrink-0">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--text-secondary)] flex items-center gap-1 mt-0.5">
              <ShieldCheck className="h-3.5 w-3.5 text-emerald-700 dark:text-emerald-400" /> Explicit feed authority & member mute controls
            </p>
          </div>
        </div>

        {/* Temporary Room Policy Notice */}
        <div className="mb-5 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3 flex items-start gap-2.5">
          <Lock className="h-4 w-4 text-emerald-700 dark:text-emerald-400 shrink-0 mt-0.5" />
        ...
        <form onSubmit={handleAddSubmit} className="mb-5 flex items-center gap-2">
          <input
            ...
            className="flex-1 rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-sunken)] px-3 py-2 text-xs text-[var(--text-primary)] placeholder-[var(--text-muted)] focus:border-[var(--accent-botanical-sage)] focus:outline-none"
          />
          <button
            type="submit"
            disabled={!newMemberName.trim()}
            className="flex items-center gap-1.5 rounded-xl bg-[var(--accent-botanical-sage)] px-3.5 py-2 text-xs font-semibold text-white hover:brightness-110 transition disabled:opacity-40 shrink-0 shadow-md"
          >
        ...
        {/* Summary Bar */}
        <div className="mb-3 flex items-center justify-between text-xs text-[var(--text-muted)] px-1 font-medium">
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      <p className="font-semibold text-xs text-[var(--text-primary)] truncate">{member.name}</p>
                      {member.isCirclePartner ? (
                        ...
                        <span className="rounded-full bg-[var(--card-hover)] px-2 py-0.5 text-[9px] font-medium text-[var(--text-muted)]">
                          Temporary Guest
                        </span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  {member.isCirclePartner ? (
                    <>
                      ...
                      >
                        {member.isMuted ? (
                          ...
                          <>
                            <Volume2 className="h-3 w-3 text-emerald-700 dark:text-emerald-400" />
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 rounded-lg border border-red-500/30 bg-red-500/10 px-2 py-1 text-[11px] font-medium text-red-500 hover:bg-red-500/20 transition"
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 rounded-lg bg-emerald-500/15 border border-emerald-500/40 px-2.5 py-1 text-[11px] font-bold text-emerald-700 dark:text-emerald-400 hover:bg-emerald-500/25 transition"
                    >
        ...
        <div className="mt-5 pt-3 border-t border-[var(--border-subtle)] flex justify-end">
          <button
            onClick={onClose}
            className="rounded-xl bg-[var(--accent-botanical-sage)] px-4 py-2 text-xs font-semibold text-white hover:brightness-110 transition shadow-md"
          >
            Done
          </button>
        </div>
